...
// Retrieve the task value from the session variable
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['edit-task'])) {
        ...
        try {
            global $conn;
            $stmt = $conn->prepare("SELECT task FROM todolist WHERE id = :id");
            $stmt->bindParam(':id', $editTask);
            $stmt->execute();
            $row = $stmt->fetch();
            $taskValue = $row['task'];

            echo "Edit Task ID: " . $editTask . "<br>";
            echo "Task box: " .  $_POST['textbox'];
        } catch (Exception $e) {
