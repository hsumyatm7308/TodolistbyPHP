...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update'])) {
        $updatedData = update(); // Pass the value of the update button as an argument
        // $updatedData = update($taskId);


        echo $updatedData; // Output the updated value

        
    // Redirect the user back to index.php with the updated taskId and data as query parameters
    $taskId = $updatedData['taskId'];
    $data = $updatedData['data'];
    header("Location: index.php?taskId=" . urlencode($taskId) . "&updatedData=" . urlencode($data));
    exit();
    }
}
