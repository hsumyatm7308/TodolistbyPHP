...
$completedIds = []; // Array to store completed task IDs
...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        $completedIds[] = $comid; // Add completed task ID to array
    } catch (Exception $e) {
        echo "Found Error: " . $e->getMessage();
    }
}
...
<?php while ($row = $stmt->fetch()): ?>
    <?php if (!in_array($row['id'], $completedIds)): ?> <!-- Skip displaying completed tasks -->
            ...
            <div class="w-[85%] h-auto bg-stone-200 flex justify-between items-center p-3">
                ...
                <div class="flex justify-center items-center">
                    ...
                    <form action="deletetodo.php" method="post" class="flex justify-center items-center">
                        <button type="submit" name="delete-task" value="<?php echo $row['id']; ?>"
                            class="delete-task">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="currentColor" class="w-6 h-6">
                                <path stroke-linecap="round" stroke-linejoin="round"
