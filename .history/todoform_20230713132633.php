...
if (isset($_POST['complete-check'])) {
    $completedTaskId = $_POST['complete-check'];
    try {
        $completeStmt = $conn->prepare("UPDATE todolist SET complete = 1 WHERE id = :completedTaskId");
        $completeStmt->bindParam(":completedTaskId", $completedTaskId);
        $completeStmt->execute();

        // Prepare a new statement object to fetch the updated task list
        $updatedStmt = $conn->prepare("SELECT id, task FROM todolist WHERE complete = 0 ORDER BY id ASC");
        $updatedStmt->execute();

        // Assign the new statement object to a different variable
        $stmt = $updatedStmt;
    } catch (Exception $e) {
        echo "Error: " . $e->getMessage();
    }
        ...
        <div class="w-[85%] h-auto bg-stone-200 flex justify-between items-center p-3">
            <span class="flex justify-center items-center">
                <form action="" method="post">
                    <button name="complete-check" value="<?php echo $row['id']; ?>">
