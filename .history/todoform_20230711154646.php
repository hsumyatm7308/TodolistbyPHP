    ...
    <div name="taskitems" class="w-full h-auto flex justify-center items-center p-1">
        <div class="w-[85%] h-auto bg-stone-200 flex justify-between items-center p-3">
            ...
            <div class="flex justify-center items-center">
                <button class="mr-4 editbtn" data-taskid="<?php echo $row['id']; ?>">
...
<script>
    ...
    for (var x = 0; x < editbtn.length; x++) {
        editbtn[x].addEventListener('click', function (event) {
            const submitt =document.getElementById('submitBtn');
            submitt.setAttribute('name','update');
            const editTaskId = this.getAttribute('data-taskid');

            // Make an AJAX request to create.php with the editTaskId
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    // Handle the response from create.php
                    console.log(this.responseText);
                }
            };
            xhttp.open("POST", "create.php", true);
            xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xhttp.send("update=1&id=" + editTaskId);
        });
    }
</script>
