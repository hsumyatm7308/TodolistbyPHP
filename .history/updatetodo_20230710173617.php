...
<script>
    ...
    for (let i = 0; i < deleteButtons.length; i++) {
        deleteButtons[i].addEventListener('click', function () {
            const taskId = deleteButtons[i].getAttribute('value');
            removeTask(taskId);
        });
    }

    function removeTask(taskId) {
        const taskItem = document.querySelector(`.task-item[value="${taskId}"]`);
        if (taskItem) {
            taskItem.remove();
            console.log(`Task with ID ${taskId} removed`);
        }

        // Send AJAX request to delete the task from the server
