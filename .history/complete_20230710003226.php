...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["completeid"])) {
    ...
    try {
        ...
        $cpstmt = $conn->prepare("INSERT INTO todolist(complete) VALUE (:complete) ");

        $stmt = $conn->prepare("DELETE FROM todolist WHERE id = $completeid ");

        $cpstmt->bindParam(':complete',$row['task']);
