    ...
    <div name="taskitems" class="w-full h-auto flex justify-center items-center p-1">
        <div class="w-[85%] h-auto bg-stone-200 flex justify-between items-center p-3">
            ...
            <div class="flex justify-center items-center">
                <button class="mr-4 editbtn" name="edit-task" value="<?php echo $row['id']; ?>" onclick="updateTask('<?php echo $row['id']; ?>')">
                ...
                <form action="deletetodo.php" method="post" class="flex justify-center items-center">
                    <input type="hidden" name="edit-task" id="editTaskId">
...
<script>
    ...
    for (var x = 0; x < editbtn.length; x++) {
        editbtn[x].addEventListener('click', function (event) {
            const editTaskId = event.target.value;
            document.getElementById('editTaskId').value = editTaskId;
        });
    }

    function updateTask(taskId) {
        // Make an AJAX request to update.php with the taskId
        var xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                // Handle the response from update.php
                var updatedTaskId = this.responseText;
                console.log(updatedTaskId);
                // Display the updated taskId in the desired location
                var updatedTaskIdElement = document.getElementById('updatedTaskId');
                if (updatedTaskIdElement) {
                    updatedTaskIdElement.innerHTML = updatedTaskId;
                }
            }
        };
